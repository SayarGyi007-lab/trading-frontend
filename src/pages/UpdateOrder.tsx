...
function UpdateOrder() {
  const { data: ordersData } = useGetAllOrderQuery()
  const { data: productsData, isLoading: isProductsLoading } = useGetAllProductsQuery()
  const [updateOrder, { isLoading }] = useUpdateOrderMutation()
  const { orderId } = useParams<{ orderId: string }>()
  const navigate = useNavigate()

  const order = ordersData?.orders.find(o => o.order_id === Number(orderId))
  ...
  if (!order) return <p className="text-center text-gray-200 mt-10">Order not found.</p>
  if (isProductsLoading) return <p className="text-center text-gray-200 mt-10">Loading products...</p>
        ...
        <div className="flex flex-col md:flex-row gap-12 items-stretch">
          ...
          <div className="md:w-1/2 flex items-center">
            <div className="w-full p-8 bg-black/30 backdrop-blur-md rounded-3xl shadow-2xl flex flex-col gap-4">
              <h3 className="text-2xl font-bold text-white mb-4 text-center">Update Order Details</h3>
